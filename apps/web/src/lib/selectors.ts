...
type ProfileWithEvents<T extends ResolvedEvent> = Omit<ResolvedProfile, 'events'> & { events: T[] }

export function selectDiscover<T extends ResolvedEvent>(profile: ProfileWithEvents<T>, marketSources: ReadonlySet<string>, now = new Date(), seed = 'stable', exposures: Readonly<Record<string, ExposureSignal>> = {}) {
  ...
  }).map((item) => item.event as T)
}

export function selectMine<T extends ResolvedEvent>(profile: ProfileWithEvents<T>, now = new Date()) {
  ...
  return profile.events.filter((event) =>
    ...
    (event.sourceLocators.some((s) => s.startsWith('local:')) || ids.has(event.id) || event.sourceLocators.some((source) => sources.has(source))),
  ).sort((a, b) => {
