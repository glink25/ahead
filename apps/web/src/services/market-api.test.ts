...
it('restores a bounded local working set without requesting Market', async () => {
  const storage = memory()
  const cacheStore = memory()
  const listing = {
    ...issue(1),
    source: {
      schema: 1 as const,
      locator: 'github:a/r1',
      manifestPath: 'ahead.yaml',
      resourceType: 'event-feed' as const,
    },
    issueNumber: 1,
  ...
  await storage.set('market:a/market', [listing])
  await storage.set('market-probe:a/market', { checkedAt: Date.now() })
  const cache = new RepoCache(cacheStore)
  await cache.write({
    sourceLocator: 'github:a/r1', manifestPath: 'ahead.yaml',
    headSha: sha, feed: feed(),
  })
  const fetcher = vi.fn(async (input: RequestInfo | URL) => response(input))
  const api = new MarketApi({
    repository: 'a/market', storage, cache,
    client: new PublicReadClient({ fetcher, store: memory(), authenticated: true, apiInterval: 0 }),
  })
  let available = 0
  const statuses: string[] = []
  const session = api.market.openSession({
    receive: (event) => { if (event.type === 'feed') available++ },
    status: (status) => statuses.push(status),
    available: () => available,
  })
  session.setActive(true)
  await vi.waitFor(() => expect(statuses.at(-1)).toBe('paused'))
  expect(available).toBe(1)
  expect(fetcher.mock.calls.some(([url]) => String(url).includes('/issues?'))).toBe(false)
  session.close()
})

it('keeps paging thresholds inside the API session', async () => {
  const pages: number[] = []
  const fetcher = vi.fn(async (input: RequestInfo | URL) => {
    const url = String(input)
    if (url.includes('/issues?')) {
      ...
      pages.push(page)
      return new Response(JSON.stringify([issue(page)]), {
  ...
  session.setActive(true)
  await vi.waitFor(() => expect(pages).toEqual([1]))
  session.reportVisible(9, 12)
  await new Promise((resolve) => setTimeout(resolve, 0))
  expect(pages).toEqual([1])
  session.reportVisible(10, 12)
  await vi.waitFor(() => expect(pages).toEqual([1, 2]))
  session.close()
})

it('probes only Market metadata once within six hours', async () => {
  const fetcher = vi.fn(async (input: RequestInfo | URL) => {
    if (String(input).includes('/issues?'))
  ...
  const api = setup(fetcher)
  expect(await api.market.probeHead()).toHaveLength(1)
