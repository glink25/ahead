...
describe('handleRequest OAuth flow', () => {
  ...
  it('diverts to App installation with pending_token sealed in state', async () => {
    const state = await encryptState({
      redirect_uri: 'http://localhost:4455/login',
      exp: Date.now() + 60_000,
    }, env.STATE_SECRET)
    ...
    const installState = await decryptState(location.searchParams.get('state')!, env.STATE_SECRET)
    expect(installState.redirect_uri).toBe('http://localhost:4455/login')
    expect(installState.pending_token).toEqual({
      access_token: 'ghu_access',
      refresh_token: 'ghr_refresh',
      expires_in: 3_600,
      refresh_token_expires_in: 86_400,
  ...
  it('returns github_authorized from Setup URL using pending_token in state', async () => {
    const state = await encryptState({
      redirect_uri: 'http://localhost:4455/login',
      exp: Date.now() + 60_000,
      pending_token: {
    ...
    const response = await handleRequest(
      new Request(
        `https://auth.example/api/github/callback?installation_id=42&setup_action=install&state=${encodeURIComponent(state)}`,
      ),
      env,
    )
    ...
    const location = new URL(response.headers.get('Location')!)
    expect(location.origin + location.pathname).toBe('http://localhost:4455/login')
    expect(JSON.parse(location.searchParams.get('github_authorized')!)).toMatchObject({
      access_token: 'ghu_access',
      refresh_token: 'ghr_refresh',
    })
  })

  it('rejects Setup URL return when pending_token is missing', async () => {
    const state = await encryptState({
      redirect_uri: 'http://localhost:4455/login',
      exp: Date.now() + 60_000,
    }, env.STATE_SECRET)
    ...
    expect(response.status).toBe(400)
    expect(await response.text()).toBe('Missing pending_token for installation return')
  })

  it('rejects expired Setup URL state', async () => {
    const state = await encryptState({
      redirect_uri: 'http://localhost:4455/login',
      exp: Date.now() - 1,
      pending_token: { access_token: 'ghu_access' },
    }, env.STATE_SECRET)
    ...
    expect(response.status).toBe(400)
    expect(await response.text()).toBe('GitHub OAuth callback failed')
  })
